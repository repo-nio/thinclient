/*
 * File version : build 19
 */
// var ExternalFn = null;
var ExternalFn = "http://shrikant.nixxis.com:8088/data";
var txWeb =
{
	...
	Init : function()
	{
		...
		else if (navigator.userAgent.indexOf("Gecko") >= 0) { this.FF = true; }
...
var NixxisContactLink = 
{
    ...
    Init : function()
    {
		if (txWeb.FF) {
			try {
				...
				document.domain = "http://shrikant.nixxis.com:8088/data";
			} 
			catch (e) {
				;
			}
		...
		if (_winExt)
		{
			...
			try 
			{
				NixxisContactLink.contactInfo.AgentAccount = window.external.UserAccount;
				NixxisContactLink.contactInfo.To = window.external.To;
				NixxisContactLink.contactInfo.From = window.external.From;
				NixxisContactLink.contactInfo.Context = window.external.Context;
				NixxisContactLink.contactInfo.Media = window.external.Media;
			}
			catch(e){}
			...
			for (var i = 0; i < _Values.length; i++) 
			{
				...
				if (_Pair[0].toLowerCase() == "ctlid") {
					NixxisContactLink.contactlistId = NixxisContactLink.__CleanContactListId(_Pair[1]);
				}
    ...
    __CleanContactListId : function(conactlistid)
	{
		if(!conactlistid) return conactlistid;
		if(conactlistid.length > 0)
		{
			if(conactlistid.substring(0,1) == '$')
			{
				return conactlistid.substring(1);
			}
		}
		return conactlistid;
	},
	ExecuteCommand : function ()
	{
			if (NixxisContactLink.isWin) 
				return window.external.executeCommand(arguments);
			else 
				return NixxisContactLink.clientLink.executeCommand.apply(NixxisContactLink.clientLink, arguments);	
	},
	agent :
    {
        UserName    : function () 
		{ 
			if (NixxisContactLink.isWin)
				return window.external.UserName;
			else 
				if (!NixxisContactLink.clientLink) return "Jane Doe"; return NixxisContactLink.clientLink.UserName; 
		},
        UserAccount : function () 
		{ 
			if (NixxisContactLink.isWin)
				return window.external.UserAccount;
			else 
				if (!NixxisContactLink.clientLink) return "100"; return NixxisContactLink.clientLink.UserAccount; 
		},
		Description : function () 
		{ 
			if (NixxisContactLink.isWin)
				return window.external.UserAccount + ", " + window.external.UserName;
			else 		
				if (!NixxisContactLink.clientLink) return "100, Jane Doe"; return NixxisContactLink.clientLink.Description; 
		},
    ...
    commands :
    {
		SearchMode : function(activityId)
		{
			ExternalFn.SearchMode(activityId);
		},
		...
		{
			ExternalFn.voicenewcall(destination); 
		},
		...
		{ 
			if (NixxisContactLink.isWin)
				ExternalFn.voiceforward(destination, NixxisContactLink.contactId);
			else
				ExternalFn.voiceforward(destination); 
		},
		voicehangup : function(destination) 
		{ 
			ExternalFn.voicehangup(); 
		},
		...
		startRecording : function()
       	{
			//NixxisContactLink.clientLink.connection.executeCommand(NixxisContactLink.clientLink.codes.commandCodes.VoiceRecord, 'True', NixxisContactLink.contactId);
			if (NixxisContactLink.isWin) 
				return window.external.executeCommand(19, NixxisContactLink.contactId, 'True');
			else 
        ...
        stopRecording : function()
      	{
			if (NixxisContactLink.isWin) 
				return window.external.executeCommand(19, NixxisContactLink.contactId, 'False');
			else 
	...
	Qualification : 
	{
		...
		callbackByPreviousAgent : function(qualificationId)
		{
			if (NixxisContactLink.isWin) 
				window.external.callbackByPreviousAgent(qualificationId);
			else 
	...
	Agenda:
	{
		...
		storeAppointmentByContact : function(dateTime, duration, description, area)
		{
			...
			if (rtn != "")
			{
				...
				{
					////debugger;
					NixxisContactLink.Agenda.AppMemberDescription = unescape(list[3]);
					NixxisContactLink.Agenda.AppAreaDescription = unescape(list[4]);
					NixxisContactLink.Agenda.AppContextDescription = unescape(list[5]);
					NixxisContactLink.Agenda.AppDuration = list[6];
					NixxisContactLink.Agenda.AppMailbox = unescape(list[7]);
				}
				return true;
			}
			return false;
	...
	Cmd :
	{
		...
		{
			if (!NixxisContactLink.clientLink) return;
			NixxisContactLink.clientLink.SearchMode.execute(activityId);
		},		
		//
		//Info commands
		//
